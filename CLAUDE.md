...
> 詳細寫作公約寫在 [`.claude/Docs/`](.claude/Docs/) 下各規定檔。
...
- **你們共同維護**：`CLAUDE.md`（這份檔案）、`.claude/skills/`（三大操作 SOP）、`.claude/Docs/` 下各規定檔、`prompts/`（轉介薄殼）、根目錄 `AGENTS.md`。

**人類掌舵，你執行。** 使用者負責找 source、提問題、定方向；你負責讀、寫、整理、跨引、查重、補洞。

## 三大操作（對應 `.claude/skills/` 三個 skill）

| 觸發語 | 你執行 | 詳細 SOP |
|-------|--------|---------|
| 「處理 `raw/<X>`」「ingest `<X>`」 | 讀 source → 跟使用者確認重點 → 寫 `wiki/summaries/` 一頁 → 更新或新建相關 `entities/` `concepts/` → 更新 `wiki/index.md` → 在 `wiki/log.md` append 一筆 | [.claude/skills/ingest/SKILL.md](.claude/skills/ingest/SKILL.md) |
| 「問 …」「查 …」「回答 …」 | 先讀 `wiki/index.md` → 讀相關頁 → 綜合作答（附引用） → 問使用者是否要回填成 `wiki/syntheses/` 一頁 | [.claude/skills/query/SKILL.md](.claude/skills/query/SKILL.md) |
| 「lint wiki」「健康檢查」 | 找矛盾 / 孤兒頁 / 過期主張 / 缺頁 / 缺交叉引用 / 可補的網搜題目 → 寫成報告給使用者決策 | [.claude/skills/lint/SKILL.md](.claude/skills/lint/SKILL.md) |
...
- 要寫 wiki 頁 → 先讀 [.claude/Docs/wiki-conventions.md](.claude/Docs/wiki-conventions.md)，再讀目標頁類型的規定（`.claude/Docs/wiki-entities.md` / `wiki-concepts.md` / `wiki-summaries.md` / `wiki-syntheses.md`）。
- 要 ingest 新 source → 讀 [.claude/skills/ingest/SKILL.md](.claude/skills/ingest/SKILL.md)。
- 要 lint → 讀 [.claude/skills/lint/SKILL.md](.claude/skills/lint/SKILL.md)，然後跑 `bash scripts/check-consistency.sh` 看機械化檢查結果。
...
- **不要**把這個檔案寫超過 ~100 行。地圖要精簡，細節放到 `.claude/Docs/` 下各規定檔。
